...
class clsbpFileDetails extends clsbpBase  { 
	...
	function setPostVars() {	
		...
		if (isset($_POST['mS']))  $this->mS	            = $_POST['mS'];
	...
	function setGetVars() {
		...
		if (isset($_GET['mS']))      $this->mS	            = $_GET['mS'];
...
if (trim($this->seachCat) == "sellers" && trim($this->txtSearch) != "" )  {
	...
	function retrieveMemberStoreArray() {
	 	 $query = "SELECT tbl_bopabox.bi_file_id,tbl_bopabox.vc_FileName, tbl_bopabox.bi_MemberId,
				 ...
				 AND tbl_bopabox.ti_lock =0
				 AND tbl_bopabox.i_in_sell =1
				 AND memlogin.vc_DisplayName = '".$_REQUEST[mS]."'";
		 $query	.= " ORDER BY $this->sortColumn $this->sortDirection";
...
tbl_mem_login.vc_DisplayName ='".$_REQUEST[mS]."'";	
...
print_r($_REQUEST);*/
		...
		if ($this->action == "SAVE") {		
			...
			if($this->updateSongDetails($this->bi_file_id) > 0)
			{
				...
				header("location:".stripslashes($this->returnURL));
				//echo "<script language=\"javascript\" type=\"text/javascript\">document.href=\"".stripslashes($this->returnURL)."\"</script>";
				exit();	
			}
			...
			if($this->updateSongStatusDetails($this->bi_file_id) > 0)
			{
				...
				header("location:".stripslashes($this->returnURL)."&this_file_id=".$this->bi_file_id);
				exit();	
			}
