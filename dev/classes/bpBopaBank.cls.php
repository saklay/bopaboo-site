...
if($this->sortColumn == "vc_EmailId" || $this->sortColumn =="f_WithdrawalAmount" ||  $this->sortColumn =="f_BopabankBalance" || $this->sortColumn =="d_withdrawalDate")
 ...
 */
 	 $query = "(SELECT history.bi_HistoryId, sum( history.vc_filePrice ) AS sum, count( history.bi_FileId ) AS count,
				 ...
				 AND (history.dt_insertedDate > date_sub( now( ) , INTERVAL $optionVal0 DAY ))
...
$this->sortDirection    = "DESC";
	...
	if($_GET['optionVal0'] !=''){
		...
		$query .="AND (history.dt_insertedDate > date_sub( now( ) , INTERVAL $optionVal0 DAY ))";
		}
		else{
		$year = date("Y");
...
$this->sortDirection    = "DESC";
	 ...
	 */
	   ...
	   return $this->fn_RetriveOrderDetailsArray($dbQry);
		...
		function retrieveWithdrawalArray()
		{
		$optionVal0 =$_GET['optionVal0'];
		if($optionVal0==""){
		$optionVal0 = 30;
		...
		$this->sortColumn  = "d_withdrawalDate";
		$this->sortDirection = "DESC";
		}
		if($_REQUEST['sortColumn']=='ord_date'){
		$this->sortColumn  = "d_withdrawalDate";
		$this->sortDirection = "DESC";
		}
		$query	= " SELECT * from tbl_withdrawal where  bi_MemberId = $this->bi_MemberId AND (d_withdrawalDate > date_sub( now( ) , INTERVAL $optionVal0 DAY ))";
		$query	.= " ORDER BY $this->sortColumn  $this->sortDirection "; 
	}
	...
	
	{
		...
		$this->sortColumn  = "d_withdrawalDate";
		$this->sortDirection = "DESC";
		$query	= " SELECT * from tbl_withdrawal where  bi_MemberId = $this->bi_MemberId  AND YEAR(d_withdrawalDate)='$year' ";
		...
		return $this->retrieveWithDrawalrRowArray($this->clsbpPaginate);
		}
		
		function retrieveWithDrawalrRowArray($dbQry){
			
			$arrDetails	= array();
			while($row  = $dbQry->getArray()) {
			
			$arrDetails[$row["bi_withdrawal_id"]]	= array	(
											"bi_withdrawal_id"	    => $row["bi_withdrawal_id"],
											" bi_MemberId"	        => $row["bi_MemberId"],	
											"vc_EmailId"	    => $row["vc_EmailId"],
											"f_WithdrawalAmount"	    => $row["f_WithdrawalAmount"],
											"f_BopabankBalance"	    => $row["f_BopabankBalance"],
											"d_withdrawalDate"	    => $row["d_withdrawalDate"]
