...
class clsbpBopaCart extends clsbpBase  { 
	...
	function setGetVars() {
		...
		if (isset($_GET['cardId']))	$this->cardId = trim($_GET['cardId']);	
	...
	function retrieveFileDetailsOfUser($FileID) {
		...
		$query ="SELECT * FROM tbl_mem_details mem, tbl_bopabox bpbox, tbl_id3_tags id3
					WHERE bpbox.i_tg_id = id3.i_tg_id 
					AND bpbox.bi_file_id = $FileID"; 						 
						...
						{			
						   ...
						   if(isset($_SESSION['bopaBasket']['song'])){
										   ...
										   for($i=1;$i<=$cartSize;$i++){
											...
											$fileDetails = $this->retrieveFileDetailsOfUser($FileID);
											$TotalPrice  = $TotalPrice +   number_format($fileDetails['dbl_price'],2);	
											}
							}
						}
						 $TotalPrice;
...
/////////////// Cart total amount ends here //////////
	...
	function postCart() {	
		...
		if ($this->action == "ADDTOCART") {		
			...
			if($this->cardId<>''){		
			$this->returnUrl = $this->returnUrl."&cardId=".$this->cardId;
			}
			$this->fn_removeItem();
			header("location:".stripslashes($this->includePath.$this->returnUrl));				
			exit();
