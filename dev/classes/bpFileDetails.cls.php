...
class clsbpFileDetails extends clsbpBase  { 
	...
	var $countFiles;
	var $pageSize;
	var $stroeSize; // Page size for MyStore
	var $tabName;
	var $feedSortcolumn;
	...
	var $flagAdvSearch;  // Flag, which decides whether the search is 
	...
	function clsbpFileDetails($connect, $includePath) {			
		...
		$this->feedSortcolumn		= "dt_insertedDate";
		$this->feedSortDirection    = "DESC";
		$this->pageSize			= 20;
		$this->searchSize      = 20;
		$this->storeSize      = 10; // Page Size for Store
		...
		$this->priceTotalSellYear = 0;
 		...
 		$this->flagTopSearch = 0;
		$this->artistname ="";
		$this->albumname ="";
		
		$this->flagAdvSearch = "OFF";
	}
	...
	function setPostVars() {	
		...
		if(isset($_POST['clsbpFileDetails_feedSortDirection'])) 	  $this->feedSortDirection			=	$_POST['clsbpFileDetails_feedSortDirection'];
		if(isset($_POST['clsbpFileDetails_feedSortcolumn'])) 	 $this->feedSortcolumn			=	$_POST['clsbpFileDetails_feedSortcolumn'];
		...
		if (isset($_POST['clsbpFileDetails_flagTopSearch']))  $this->flagTopSearch	            = $_POST['clsbpFileDetails_flagTopSearch'];
		...
		if (isset($_POST['mS']))  $this->mS	            = str_replace("-"," ",$_POST['mS']);
		if(isset($_POST['clsbpFileDetails_artist'])) 	 $this->artistname		=	$_POST['clsbpFileDetails_artist'];
		if(isset($_POST['clsbpFileDetails_album'])) 	 $this->albumname			=	$_POST['clsbpFileDetails_album'];
		if(isset($_POST['clsbpFileDetails_advSearchFlag'])) 	 $this->flagAdvSearch			=	$_POST['clsbpFileDetails_advSearchFlag'];
	...
	function setGetVars() {
		...
		if (isset($_GET['clsbpFileDetails_flagTopSearch']))  $this->flagTopSearch	            = $_GET['clsbpFileDetails_flagTopSearch'];
		...
		if (isset($_GET['mS']))      $this->mS	            = str_replace("-"," ",$_GET['mS']);
		
		if(isset($_GET['clsbpFileDetails_feedSortDirection'])) 	  $this->feedSortDirection			=	$_GET['clsbpFileDetails_feedSortDirection'];
		if(isset($_GET['clsbpFileDetails_feedSortcolumn'])) 	 $this->feedSortcolumn			=	$_GET['clsbpFileDetails_feedSortcolumn'];
		
		if(isset($_GET['clsbpFileDetails_artist'])) 	 $this->artistname		=	$_GET['clsbpFileDetails_artist'];
		if(isset($_GET['clsbpFileDetails_album'])) 	 $this->albumname			=	$_GET['clsbpFileDetails_album'];
               if (isset($_GET['clsbpFileDetails_jumpTo']))            $this->jumpTo	        = trim($_GET['clsbpFileDetails_jumpTo']);
		if(isset($_GET['clsbpFileDetails_sortColumn'])) 	  $this->sortColumn			=	$_GET['clsbpFileDetails_sortColumn'];
               	if(isset($_GET['clsbpFileDetails_searchSize'])) 	  $this->searchSize			=	$_GET['clsbpFileDetails_searchSize'];
               if(isset($_GET['clsbpFileDetails_sortDirection'])) 	  $this->sortDirection			=	$_GET['clsbpFileDetails_sortDirection'];
               if(isset($_GET['clsbpFileDetails_bi_GenreId'])) 	  $this->bi_GenreId			=	$_GET['clsbpFileDetails_bi_GenreId'];
		if(isset($_GET['clsbpFileDetails_seachCat'])) 	  $this->seachCat			=	$_GET['clsbpFileDetails_seachCat'];
		if(isset($_GET['clsbpFileDetails_seachCat'])) 	  $this->seachCat			=	$_GET['clsbpFileDetails_seachCat'];
		if(isset($_GET['clsbpFileDetails_goBack'])) 	  $this->goBack			=	$_GET['clsbpFileDetails_goBack'];
		
		if(isset($_GET['clsbpFileDetails_advSearchFlag'])) 	 $this->flagAdvSearch			=	$_GET['clsbpFileDetails_advSearchFlag'];
	}
...
// retrive a single row 
	...
	function retrieveFileRow($dbQry) {
	    ...
	    $this->memberRegDate=$row['dt_SignUp'];	
		return 1;
	}
	...
	function retrieveFileArray() { 	
		...
		$query = "SELECT 
					t1. * , 
					count( * ) AS count, 
					avg( vc_rating ) AS avgRating 
				FROM (";
				
		$query .= "SELECT tbl_bopabox.bi_file_id,tbl_bopabox.vc_FileName, tbl_bopabox.bi_MemberId, tbl_bopabox.dbl_price, tbl_bopabox.i_in_sell, tbl_bopabox.ti_lock, tbl_id3_tags.vc_title_nm_mod, tbl_id3_tags.vc_artists_nm_mod, tbl_bopabox.dt_FileUploaded,  tbl_id3_tags.vc_album_nm_mod,tbl_id3_tags.vc_tags,tbl_bopabox.bi_GenreId,tbl_id3_tags.f_bitrate,lkup_genre.vc_GenreName
					FROM tbl_bopabox
					INNER JOIN  tbl_mem_login memlogin  ON memlogin.bi_MemberId = tbl_bopabox.bi_MemberId
...
if (trim($this->seachCat) == "sellers" && trim($this->txtSearch) != "" )  {
			$query.=  " AND (memlogin.vc_DisplayName LIKE '%".$this->txtSearch."%')";
		}

		if ((trim($this->seachCat) == "all") || (trim($this->seachCat) == "" && trim($this->txtSearch)!="" && trim($this->txtSearch)!="Search All Music") ) {
			$query .= " AND (tbl_id3_tags.vc_artists_nm_mod LIKE '%".$this->txtSearch."%' OR tbl_id3_tags.vc_album_nm_mod LIKE '%".$this->txtSearch."%' OR tbl_id3_tags.vc_title_nm_mod LIKE '%".$this->txtSearch."%' OR lkup_genre.vc_GenreName = '".$this->txtSearch."' OR tbl_id3_tags.vc_tags LIKE '%".$this->txtSearch."%'  OR lkup_genre.vc_GenreName = '".$this->txtSearch."')";
			
		}
		 if (trim($this->artistname) != "") 
			$query	.= " AND tbl_id3_tags.vc_artists_nm_mod LIKE '%".$this->artistname."%' ";
			 if (trim($this->albumname) != "") 
			$query	.= " AND tbl_id3_tags.vc_album_nm_mod LIKE '%".$this->albumname."%' ";
	...
	$query .= ") AS t1
					LEFT JOIN 
						tbl_ratings 
							ON 
						tbl_ratings.bi_SellerId = t1.bi_MemberId
				 	GROUP BY t1.bi_file_id";
...
//print $query;
	...
	$this->pageIndex=$this->jumpTo;
		...
		$this->clsbpPaginate = new clsbpPaginate($this->connect->connId, $query, $this->searchSize, $this->rangeVal, $this->pageIndex, $this->includePath, 1);	
		
		return $this->retrieveFileRowArray($this->clsbpPaginate);
	}
	
	function retrieveAdvFileTopArray($title, $artist) {
		$query = "SELECT 
					t1. * , 
					count( * ) AS count, 
					avg( vc_rating ) AS avgRating 
				FROM (";
		$query .= "SELECT 
					tbl_bopabox.bi_file_id,
					tbl_bopabox.vc_FileName, 
					tbl_bopabox.bi_MemberId, 
					tbl_bopabox.dbl_price, 
					tbl_bopabox.i_in_sell, 
					tbl_bopabox.ti_lock, 
					tbl_bopabox.dt_FileUploaded, 
					tbl_id3_tags.vc_title_nm_mod, 
					tbl_id3_tags.vc_artists_nm_mod, 
					tbl_id3_tags.vc_album_nm_mod,
					tbl_id3_tags.vc_tags,
					tbl_bopabox.bi_GenreId,
					tbl_id3_tags.f_bitrate,
					lkup_genre.vc_GenreName
				FROM 
					tbl_bopabox INNER JOIN  tbl_mem_login memlogin  
						ON memlogin.bi_MemberId = tbl_bopabox.bi_MemberId
					INNER JOIN tbl_id3_tags 
						ON tbl_id3_tags.i_tg_id = tbl_bopabox.i_tg_id
					INNER JOIN lkup_genre 
						ON lkup_genre.bi_GenreId = tbl_bopabox.bi_GenreId
				WHERE 
					1 =1
						AND 
					tbl_bopabox.ti_lock =0
					...
					tbl_bopabox.i_in_sell =1
						AND
					tbl_id3_tags.vc_artists_nm_mod = '$artist'
						AND
					tbl_id3_tags.vc_title_nm_mod = '$title'
					";
		if(isset($_SESSION['USERID'])) {
			$query .= " AND tbl_bopabox.bi_MemberId !=".$_SESSION['USERID'];
		}
	...
	/* Function to retrive member's files which are posted for sale
	called at bpMemberStore.php
	 */
	function retrieveMemberStoreArray() {
	 	 $query = "SELECT tbl_bopabox.bi_file_id,tbl_bopabox.vc_FileName, tbl_bopabox.bi_MemberId,
		         tbl_bopabox.dbl_price,tbl_bopabox.i_in_sell, tbl_bopabox.ti_lock, tbl_id3_tags.vc_title_nm_mod,
				 tbl_id3_tags.vc_artists_nm_mod,tbl_id3_tags.vc_album_nm_mod,tbl_id3_tags.vc_tags,tbl_bopabox.bi_GenreId,
				 tbl_id3_tags.f_bitrate,lkup_genre.vc_GenreName
				 FROM tbl_bopabox
				 INNER JOIN  tbl_mem_login memlogin  ON memlogin.bi_MemberId = tbl_bopabox.bi_MemberId
				 INNER JOIN tbl_id3_tags ON tbl_id3_tags.i_tg_id = tbl_bopabox.i_tg_id
				 INNER JOIN lkup_genre ON lkup_genre.bi_GenreId = tbl_bopabox.bi_GenreId
				 ...
				 AND tbl_bopabox.ti_lock =0
				 AND tbl_bopabox.i_in_sell =1
				 AND memlogin.vc_DisplayName = '".str_replace("-"," ",$_REQUEST[mS])."'";
		 $query	.= " ORDER BY $this->sortColumn $this->sortDirection";
		
		$dbQry	= new dbQuery($query, $this->connect->connId);
		$this->clsbpPaginate = new clsbpPaginate($this->connect->connId, $query, $this->storeSize, $this->rangeVal, $this->pageIndex, $this->includePath, 0);	
		return $this->retrieveFileRowArray($this->clsbpPaginate);
					
	}
	/* End of function retrieveMemberStoreArray() */
	...
	function retrieveAdvFileArray() { 
		...
		if($this->saveId == 0) {
			...
			$query	.= "SELECT 
							...
							bpbox.i_in_sell, 
							bpbox.ti_lock, 
							bpbox.dt_FileUploaded, 
							id3.vc_title_nm_mod, 
							id3.vc_artists_nm_mod, 
							id3.vc_album_nm_mod,
							id3.vc_tags,
							bpbox.bi_GenreId,
							id3.f_bitrate
			...
			if($this->searchCat !="" && $this->txtSearch !="") {
				...
				switch($this->searchCat) {
						...
						case "song": 
									...
									break;
						default 	   :
									$query .= " AND (id3.vc_artists_nm_mod LIKE '%".$this->txtSearch."%' OR id3.vc_album_nm_mod LIKE '%".$this->txtSearch."%' OR id3.vc_title_nm_mod LIKE '%".$this->txtSearch."%' OR id3.vc_tags LIKE '%".$this->txtSearch."%')";
									break;
			...
			else if(($this->searchMinAmount == $this->searchMaxAmount) && ($this->searchMinAmount !='') &&( $this->searchMaxAmount!='')) {
			...
			if($this->searchIfSeller != "") {
				...
				switch($this->searchAction) {
					...
					case '0': 	if($this->searchSeller!="") {
								$query .= " AND (member.vc_FirstName NOT LIKE '".$this->searchSeller."' OR member.vc_LastName NOT LIKE '".$this->searchSeller."'  OR memlogin.vc_DisplayName NOT LIKE '".$this->searchSeller."')";
							}
							break;
				}
...
function retrieveBopaBoxArray() { 	
	...
	//displayObject($this->clsbpPaginate) ;	
			return $this->retrieveFileRowArray($this->clsbpPaginate);
}
...
function retrieveFileRowArray($dbQry) {
 ...
 function getTopSellingSongsList() {
 	$query = "	SELECT 
 						vc_title_nm_mod, 
						vc_artists_nm_mod, 
						vc_album_nm_mod, 
				...
				FROM 
						tbl_bopabox bpbox, 
						tbl_id3_tags id3, 
						tbl_file_history history
				WHERE 
						bpbox.i_tg_id = id3.i_tg_id
							AND 
						bpbox.bi_MemberId = history.bi_BuyerId
							AND 
						bpbox.bi_file_id = history.bi_FileId
							AND
						bpbox.ti_lock = 0	
				GROUP BY 
						vc_title_nm_mod, vc_artists_nm_mod
				ORDER BY 
						cnt DESC, vc_title_nm_mod ASC";
	
	$dbQry = new dbQuery($query, $this->connect->connId);
	return $this->retrieveTopListRowArray($dbQry);
	
						
}

function retrieveTopListRowArray($dbQry) {
	$arrFile	= array();
		while($row  = $dbQry->getArray()) {
		$arrFile[$row["vc_title_nm_mod"]]	= array	(
										"vc_title_nm_mod"	    => $row["vc_title_nm_mod"],	
										"vc_artists_nm_mod"	    => $row["vc_artists_nm_mod"]									
										 );
		}			
...
function getSellPageDet($memberId) {
	$query =  "  SELECT *
						...
						SELECT bi_file_id, dbl_price, tbl_id3_tags.vc_title_nm_mod title1, tbl_id3_tags.vc_artists_nm_mod artist1
						FROM tbl_bopabox
						INNER JOIN tbl_id3_tags ON tbl_id3_tags.`i_tg_id` = tbl_bopabox.`i_tg_id`
						WHERE tbl_bopabox.bi_memberId =$memberId
						AND tbl_bopabox.i_in_sell =1
						) AS t2
						INNER JOIN (
						...
						FROM tbl_bopabox bpbox, tbl_id3_tags id3
						WHERE bpbox.i_tg_id = id3.i_tg_id
						AND bpbox.i_in_sell =1
						GROUP BY id3.vc_title_nm_mod, id3.vc_artists_nm_mod
						) AS t1 ON t2.title1 = t1.vc_title_nm_mod
						AND t2.artist1 = t1.vc_artists_nm_mod
						WHERE count_files >1";
			$query	.= " ORDER BY $this->sortColumn $this->sortDirection"; 
			$this->clsbpPaginate = new clsbpPaginate($this->connect->connId, $query, $this->pageSize, $this->rangeVal, $this->pageIndex, $this->includePath, 0);
...
function checkSongsinSale($memberId)

{
 $query = "SELECT bi_file_id, dbl_price
						FROM tbl_bopabox
						INNER JOIN tbl_id3_tags ON tbl_id3_tags.`i_tg_id` = tbl_bopabox.`i_tg_id`
...
function deleteFile($fileId) {
	...
	unlink($this->vc_FilePath.$this->vc_FileName); //DELETE FILE
		...
		//include the S3 class
			...
			$filepath  = "bopaBox/".$_SESSION["USERID"]."/music/".$this->vc_FileName;
			$s3->deleteObject($bucket = 'bopabox', $uri = $filepath);	
...
*/
function getAllSoldItemsCount($SellerId) {
	$query = "SELECT *  FROM tbl_file_history WHERE 
...
function retrieveFeedbackReceivedArray() { 	
	...
	if($this->feedSortcolumn=='dt_insertedDate'){
		$this->feedSortcolumn = 'history.'.$this->feedSortcolumn;
		}
		
					
				 
			     $query	=" SELECT * FROM  tbl_file_history history,tbl_ratings rating,tbl_mem_login member WHERE 
				 	rating.bi_SellerId =$this->bi_MemberId 
					AND history.bi_FileId = rating.bi_fileId
					AND rating.bi_BuyerId=member.bi_MemberId ";
				$dbQry	= new dbQuery($query, $this->connect->connId);	
			...
			if($this->jumpTo !="")
	        $this->pageIndex=$this->jumpTo;		  
						 $query	.= " ORDER BY $this->feedSortcolumn $this->feedSortDirection"; 
...
JOIN tbl_mem_login ON tbl_file_history.bi_SellerId = tbl_mem_login.bi_MemberId
WHERE tbl_ratings.bi_fileId IS NULL AND tbl_file_history.bi_BuyerId=$this->bi_MemberId";*/
					$query ="SELECT tbl_file_history.dt_insertedDate, tbl_file_history.bi_SellerId , tbl_file_history.bi_BuyerId , tbl_file_history.bi_FileId , tbl_file_history.dt_insertedDate ,tbl_file_history.vc_title,
tbl_file_history.vc_artists,tbl_file_history.vc_album,
tbl_mem_login.vc_DisplayName, tbl_mem_login.bi_MemberId 
FROM tbl_file_history LEFT JOIN tbl_ratings ON tbl_file_history.bi_FileId = tbl_ratings.bi_fileId 
JOIN tbl_mem_login ON tbl_file_history.bi_SellerId = tbl_mem_login.bi_MemberId 
WHERE tbl_ratings.bi_fileId IS NULL AND tbl_file_history.bi_BuyerId=$this->bi_MemberId";
				   }	
				  $query	.= " ORDER BY $this->feedSortcolumn $this->feedSortDirection"; 
...
function retrieveFeedbackCompleteddArray() { 	
		...
		if($this->feedSortcolumn=='dt_insertedDate'){
		$this->feedSortcolumn = 'tbl_file_history.'.$this->feedSortcolumn;
		}
		       ...
		       /*  print  $query	=" SELECT *  FROM tbl_file_history, tbl_bopabox WHERE 
					...
					$query	="SELECT * FROM  tbl_file_history,tbl_ratings,tbl_mem_login WHERE 
				 	tbl_ratings.bi_BuyerId =$this->bi_MemberId 
					AND tbl_file_history.bi_FileId = tbl_ratings.bi_fileId
					AND tbl_ratings.bi_SellerId=tbl_mem_login.bi_MemberId";
...
function retrieveMember() { 	
		   ...
		   $query	= " SELECT * FROM tbl_mem_login,tbl_mem_details 
WHERE tbl_mem_login.bi_MemberId =tbl_mem_details .bi_MemberId AND
tbl_mem_login.vc_DisplayName ='".str_replace("-"," ",$_REQUEST[mS])."'";	
				
		$dbQry	= new dbQuery($query, $this->connect->connId);	
	   $wdth = $this->retrieveMemberRow($dbQry);
	
}

function retrieveMemberRow($dbQry) {
...
function checkFavSellerForMemberStore($memberid){
$userid = $_SESSION['USERID'];
	 $query = "SELECT * 
				FROM 
			  		tbl_seller_favourates 
...
print_r($_REQUEST);*/
		...
		if ($this->action == "SAVE") {		
		...
		else if ($_REQUEST['clsbpFileDetails_action'] == "UPDATE" || $this->action == "UPDATE") {				
			...
			else if ($_REQUEST['clsbpFileDetails_action'] == "UPDATESONG" || $this->action == "UPDATESONG") {				
			...
			else if ($_REQUEST['clsbpFileDetails_action'] == "UPDATESONGSTATUS" ||$this->action == "UPDATESONGSTATUS") {				
		...
		else if ($_REQUEST['clsbpFileDetails_action'] == "DELETEFILE" || $this->action == "DELETEFILE") {		
		...
		else if ($_REQUEST['clsbpFileDetails_action'] == "DELETEFILES" ||$this->action == "DELETEFILES") {				
