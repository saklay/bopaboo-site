...
class clsbpFileUpload extends clsbpBase  { 
	...
	return;
		...
		function moveToS3Bucket($file,$mId){
			...
			$s3 = new S3(SANKEY, SANPRIVATE);
	
			//retreive post variables
			$fileName      = "bopaBox/".$mId."/music/".$file;
	
		    	$fileTempName = "./temp/".$file;
	...
	function saveMp3Tags() { 
	    ...
	    foreach($myXML as $song){
			...
			$FileName = str_replace("*","-",$FileName);
			$FileName = str_replace("'","-",$FileName);
			rename("./temp/".$song->fileName,"./temp/".$FileName);
			// Move file to S3 Bucket
			$this->moveToS3Bucket($FileName,$this->memberId);
