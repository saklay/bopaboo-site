...
class clsbpMessages extends clsbpBase {
	...
	function sendMessage() {
			...
			if($this->nMessageToId != 0) {
				...
				$this->sendMessagetoAll($this->nMessageFromId);
			}
			
			return 1;
	}
	
	function sendMessagetoAll($userId) {
		...
		$query = "SELECT * FROM 
				   ...
				   WHERE
				   	ti_MemberStatus =1";
		$dbQry	= new dbQuery($query, $this->connect->connId);	
		$arrMembers =  $this->retrieveAllMemberIds($dbQry);
	...
	function retrieveAllMemberIds($dbQry) {
		$arrMembers	= array();
		while($row  = $dbQry->getArray()) {
			$arrMembers[]	= array(
												"bi_MemberId"	    => $row["bi_MemberId"]
	...
	function getNewMessageSentCount() {
		$query = "SELECT * FROM 
				...
				ON
					tbl_mem_login.bi_MemberId = tbl_messages.`bi_ToId`	
				WHERE 
						tbl_messages.bi_ToId =". $_SESSION['USERID']. " AND tbl_messages.ti_received = 0  AND tbl_messages.ti_inboxDel=0 ORDER BY dt_SentDate DESC";
		$dbQry	= new dbQuery($query, $this->connect->connId);
		return $dbQry->numRows();
	}
	
	function getMessageInboxCount() {
		$query = "SELECT * FROM 
					tbl_mem_login
				INNER JOIN 
					tbl_messages 
				ON
					tbl_mem_login.bi_MemberId = tbl_messages.`bi_ToId`	
				WHERE 
						tbl_messages.bi_ToId =". $_SESSION['USERID']. " AND tbl_messages.ti_inboxDel=0 ORDER BY dt_SentDate DESC";
		$dbQry	= new dbQuery($query, $this->connect->connId);
		return $dbQry->numRows();
	}
