	...
	function retrievePapaValidateArray() {
		...
		while($row  = $dbQry->getArray()) {
			$arrFile[$row["bi_file_id"]]	= array(
												...
												"f_amplitudeMedian"	=> $row["f_amplitudeMedian"],
												"f_amplitudeLoc"	=> $row["f_amplitudeLoc"],
												"f_set1amplitude1"	=> $row["f_set1amplitude1"],
												"f_set1amplitude2"	=> $row["f_set1amplitude2"],
												"f_set1amplitude3"	=> $row["f_set1amplitude3"],
												"f_set2amplitude1"	=> $row["f_set2amplitude1"],
												"f_set2amplitude2"	=> $row["f_set2amplitude2"],
												"f_set2amplitude3"	=> $row["f_set2amplitude3"],
												"f_set3amplitude1"	=> $row["f_set3amplitude1"],
												"f_set3amplitude2"	=> $row["f_set3amplitude2"],
												"f_set3amplitude3"	=> $row["f_set3amplitude3"],
												"f_set4amplitude1"	=> $row["f_set4amplitude1"],
												"f_set4amplitude2"	=> $row["f_set4amplitude2"],
												"f_set4amplitude3"	=> $row["f_set4amplitude3"],
												"f_set5amplitude1"	=> $row["f_set5amplitude1"],
												"f_set5amplitude2"	=> $row["f_set5amplitude2"],
												"f_set5amplitude3"	=> $row["f_set5amplitude3"]
...
function getFileInfo() {	
		...
		$resultHistory = $this->retrieveHistory();
		 ...
		 $xml .="<userFiles>";
			...
			foreach($resultset as $resultset){		  	
					...
					$xml .="<fileDetails>";
						...
						$xml .="\t\t\t".'<filePath>'.TEMPFILEPATH.'/</filePath>'."\r\n";
						...
						$xml .="\t\t\t".'<set1amplitude1>'.$resultset['f_set1amplitude1'].'</set1amplitude1>'."\r\n"; 
						$xml .="\t\t\t".'<set1amplitude2>'.$resultset['f_set1amplitude2'].'</set1amplitude2>'."\r\n";
						$xml .="\t\t\t".'<set1amplitude3>'.$resultset['f_set1amplitude3'].'</set1amplitude3>'."\r\n";
						$xml .="\t\t\t".'<set2amplitude1>'.$resultset['f_set2amplitude1'].'</set2amplitude1>'."\r\n";
						$xml .="\t\t\t".'<set2amplitude2>'.$resultset['f_set2amplitude2'].'</set2amplitude2>'."\r\n";
						$xml .="\t\t\t".'<set2amplitude3>'.$resultset['f_set2amplitude3'].'</set2amplitude3>'."\r\n";
						$xml .="\t\t\t".'<set3amplitude1>'.$resultset['f_set3amplitude1'].'</set3amplitude1>'."\r\n";
						$xml .="\t\t\t".'<set3amplitude2>'.$resultset['f_set3amplitude2'].'</set3amplitude2>'."\r\n";
						$xml .="\t\t\t".'<set3amplitude3>'.$resultset['f_set3amplitude3'].'</set3amplitude3>'."\r\n";
						$xml .="\t\t\t".'<set4amplitude1>'.$resultset['f_set4amplitude1'].'</set4amplitude1>'."\r\n";
						$xml .="\t\t\t".'<set4amplitude2>'.$resultset['f_set4amplitude2'].'</set4amplitude2>'."\r\n";
						$xml .="\t\t\t".'<set4amplitude3>'.$resultset['f_set4amplitude3'].'</set4amplitude3>'."\r\n";
						$xml .="\t\t\t".'<set5amplitude1>'.$resultset['f_set5amplitude1'].'</set5amplitude1>'."\r\n";
						$xml .="\t\t\t".'<set5amplitude2>'.$resultset['f_set5amplitude2'].'</set5amplitude2>'."\r\n";
						$xml .="\t\t\t".'<set5amplitude3>'.$resultset['f_set5amplitude3'].'</set5amplitude3>'."\r\n";
					$xml .="</fileDetails>";
			
			 }
			  
			 foreach($resultHistory as $resultHistory){		  	
	
					$xml .="<fileDetails>";
						$xml .="\t\t\t".'<fileName>'.$resultHistory['vc_FileName'].'</fileName>'."\r\n";
						$xml .="\t\t\t".'<fileType>mp3</fileType>'."\r\n";
						$xml .="\t\t\t".'<filePath>'.TEMPFILEPATH.'/</filePath>'."\r\n";
						$xml .="\t\t\t".'<uploadedDatetime>'.$resultHistory['dt_FileUploaded'].'</uploadedDatetime>'."\r\n";
						$xml .="\t\t\t".'<songName>'.$resultHistory['vc_title_nm_mod'].'</songName>'."\r\n";
						$xml .="\t\t\t".'<artistName>'.$resultHistory['vc_artists_nm_mod'].'</artistName>'."\r\n";
						$xml .="\t\t\t".'<genre>'.$resultHistory['vc_genre_mod'].'</genre>'."\r\n";
						$xml .="\t\t\t".'<comments>'.$resultHistory['lt_comment'].'</comments>'."\r\n";
						$xml .="\t\t\t".'<year>'.$resultHistory['vc_year'].'</year>'."\r\n";
						$xml .="\t\t\t".'<trackLength>'.$resultHistory['f_track_length'].'</trackLength>'."\r\n";
						$xml .="\t\t\t".'<tags>'.$resultHistory['vc_tags'].'</tags>'."\r\n";
						$xml .="\t\t\t".'<fileSize>'.$resultHistory['vc_FileSize'].'</fileSize>'."\r\n";
						$xml .="\t\t\t".'<albumName>'.$resultHistory['vc_album_nm_mod'].'</albumName>'."\r\n";
						$xml .="\t\t\t".'<amplitude1>'.$resultHistory['f_amplitude1'].'</amplitude1>'."\r\n";
						$xml .="\t\t\t".'<amplitude2>'.$resultHistory['f_amplitude2'].'</amplitude2>'."\r\n";
						$xml .="\t\t\t".'<amplitude3>'.$resultHistory['f_amplitude3'].'</amplitude3>'."\r\n";
						$xml .="\t\t\t".'<amplocation>'.$resultHistory['f_amplitudeLoc'].'</amplocation>'."\r\n";
						$xml .="\t\t\t".'<mean>'.$resultHistory['f_amplitudeMean'].'</mean>'."\r\n";
						$xml .="\t\t\t".'<median>'.$resultHistory['f_amplitudeMedian'].'</median>'."\r\n";
						$xml .="\t\t\t".'<sale>'.$resultHistory['f_track_length'].'</sale>'."\r\n";
						$xml .="\t\t\t".'<salePrice>'.$resultHistory['dbl_price'].'</salePrice>'."\r\n";
					$xml .="</fileDetails>";
...
function retrieveHistory() {
	$query	= "
				SELECT * FROM 
					...
					tbl_papa papa, 
					tbl_id3_tags id3, 
					tbl_file_history hist 
				WHERE 
					bpbox.bi_file_id=papa.bi_file_id 
						AND 
					bpbox.i_tg_id=id3.i_tg_id 
						AND 
					bpbox.bi_file_id=hist.bi_FileId 
						AND 
					hist.bi_SellerId=$this->bi_MemberId
			  ";

	$dbQry	= new dbQuery($query, $this->connect->connId);	
	return $this->retrievePapaValidateRowArray($dbQry); // Retrives all the information into an array and return it.
}
