			...
			{			
			 ...
			 $userbankAmount =  $clsbpPayment->retrieveBankAmountofUser(); // user bank amount
				 ...
				 $ccAmt                = $subTotalPrice - $userbankAmount; // remaining amount that has to be capture from CC Account
				  ...
				  if($ccAmt < constant("serviceFloor")){ // if subtotal price is less than service floor add searvice charge to CC amount
...
*/
	...
	function fn_SendMailtoSeller($orderId){
		...
		foreach($sellerArray as $seller){
				...
				/* update bank balance of seller*/
			/*	$arrBank = $this->retrieveBankAmountOwner($sell["bi_SellerId"]);
				 
	 	         $currentBalance = $arrBank['f_SalesAmount'] + $arrBank['f_DepositAmount'];
				
				 $updatequery	= "UPDATE 
							tbl_mem_order 
						SET 
							vc_seller_bopabank_balance = \"$currentBalance\"
	...
	function sendMailToBuyer(){		
		   ...
		   foreach($this->cartArray as $k =>$r){
			...
			if($subTotalPrice < serviceFloor ){
			$subTotalPrice  =  $subTotalPrice + serviceCharge;
			}
			$slist.='<div class="total" style="float:left; width:531px; margin:0 auto; padding:15px 28px 10px 35px; font: 11px/18px Verdana, Arial, Helvetica, sans-serif;">Service Fee:$'.serviceCharge.'</div>';
