<?php
	...
	class grabAol {
		...
		function getContactList() {	
			$cookieFile	=  '/var/tmp/'.$this->_getUsername().'_aol_cookiejar.txt';
			...
			if( file_exists( '/var/tmp/'.$this->_getUsername().'_aol_cookiejar.txt' ) ) {
				$this->_rmFile( '/var/tmp/'.$this->_getUsername().'_aol_cookiejar.txt' );
			}
			$ch		= curl_init();
			...
			foreach( $firstPhaseFilter[0] as $value ) {
				...
				$contactEmail	= $this->_parseContent( '/<span>Screen Name:<\/span> <span>(.*?)<\/span>/s', $value);
				$contactList[strip_tags($contactEmail[1][0])] = $contactName[1][0];
			}
