...
class myspace {
	function getAddressbook( $YOUR_EMAIL,$YOUR_PASSWORD ) {
		...
		@unlink( '/var/tmp/myspace_'.$YOUR_EMAIL.'_cookiejar.txt' );
		$ch = curl_init();
		//
		// setup and configure
		//
		
		curl_setopt( $ch, CURLOPT_COOKIEJAR, '/var/tmp/myspace_'.$YOUR_EMAIL.'_cookiejar.txt' );
		...
		foreach( $result['name'] as $key => $value ) {
			if( trim( $result['email'][$key] ) != '-' ) {
				$contactList[$value] = $result['email'][$key];
			}
		}
